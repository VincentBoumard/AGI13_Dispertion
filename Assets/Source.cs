...
public class Source : MonoBehaviour {
	...
	protected void Update () {
        ...
        if (isCreated && Physics.Raycast(lightPos, lightDir, out hit))
        {
            ...
            {
				//If the raycast hits a new object.
                if (oldHit != hit.transform.gameObject && oldHit != null)
                {
                    // notify the previous object it's no longer lit, and forget it
					if (oldHit.transform.gameObject.name == "Mirror" && oldHit.GetComponent<MirrorScript>() != null)
                    {
                        oldHit.GetComponent<MirrorScript>().isLit = false;

                    }
					else if (oldHit.transform.gameObject.name == "Glass" && oldHit.GetComponent<GlassScript>() != null)
                    {
                        oldHit.GetComponent<GlassScript>().isLit = false;                   
            
                    }
					else if (oldHit.transform.gameObject.name == "Target" && oldHit.GetComponent<Target>() != null)
                    {
        ...
        if (isCreatedD)
        {
            for (int i = 0; i < 4; i++)
            {
                if (Physics.Raycast(lightPos, lightDirD[i], out hitD[i]))
                {
                    if (hitD[i].transform.gameObject.name == "Mirror")
                    {
                        // store the game object we're hitting, to send it a message when it's no longer the case
						if (oldHitD[i] != hitD[i].transform.gameObject && oldHitD[i] != null)
                        {
                            // notify the previous object it's no longer lit, and forget it
							if (oldHitD[i].transform.gameObject.name == "Mirror" && oldHitD[i].GetComponent<MirrorScript>() != null)
                            {
                                oldHitD[i].GetComponent<MirrorScript>().isLit = false;

                            }
							else if (oldHitD[i].transform.gameObject.name == "Glass" && oldHitD[i].GetComponent<GlassScript>() != null)
                            {
                                oldHitD[i].GetComponent<GlassScript>().isLit = false;

                            }
							else if (oldHitD[i].transform.gameObject.name == "Target" && oldHitD[i].GetComponent<Target>() != null)
                            {
                                oldHitD[i].GetComponent<Target>().isHit = false;

                            }
							oldHitD[i] = hitD[i].transform.gameObject;           

                        }
                        else{
							oldHitD[i] = hitD[i].transform.gameObject;
                            lightType = i;
							MirrorScript ms = hitD[i].transform.gameObject.GetComponent<MirrorScript>();
							ms.Reflect(hitD[i].point, hitD[i].normal, lightDirD[i], i);
                            // distance to the object hit to properly create the light beam
							distD[i] = hitD[i].distance;
                        }
                    }

					else if (hitD[i].transform.gameObject.name == "Glass")
                    {
						if (oldHitD[i] != hitD[i].transform.gameObject && oldHitD[i] != null)
                        {
                            // notify the previous object it's no longer lit, and forget it
                        ...
                        else
                        {
							...
							oldHitD[i] = hitD[i].transform.gameObject;
                            lightType = i;
							GlassScript ms = hitD[i].transform.gameObject.GetComponent<GlassScript>();
							ms.DispersOne(hitD[i].point, hitD[i].normal, lightDirD[i], i);
                            // distance to the object hit to properly create the light beam
							distD[i] = hitD[i].distance;
                        }
                    }
					else if (hitD[i].transform.gameObject.name == "Target")
                    {
						if (oldHitD[i] != hitD[i].transform.gameObject && oldHitD[i] != null)
                        {
                            // notify the previous object it's no longer lit, and forget it
                        ...
                        else
                        {
							...
							oldHitD[i] = hitD[i].transform.gameObject;
                            lightType = i;
							Target ms = hitD[i].transform.gameObject.GetComponent<Target>();
                            if (i == ms.getColour())
                            {
                                ms.Hit();
                                //ms.GetComponent<ParticleSystem>().Play();
				...
				else //if (hitD[i].collider == null)
            	{
                	distD[i] = 100.0f;
					Debug.Log ("No hit: ELSE");
					...
					if (oldHitD[i] != null)
					{
						...
						{
							oldHitD[i].GetComponent<MirrorScript>().isLit = false;
							oldHitD[i].GetComponent<SourceAux>().lightType = 4;
        ...
        else
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (oldHitD[i] != null)
                {
                    ...
                    {
                        oldHitD[i].GetComponent<GlassScript>().isLitOne = false;
                        oldHitD[i].GetComponent<GlassScript>().isLit = false;
    ...
    public void CreateDispersedLight()
    {
        ...
        if (!isCreatedD)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
				...
				if (hitD[i].collider == null) distD[i] = 100.0f;
                lightbD[i].transform.localScale = new Vector3(0.5f, 1.0f, distD[i]);
